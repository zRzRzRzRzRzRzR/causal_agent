...
- **statistic_type**（**新增，必填**）：标记这条 edge 的"统计量身份"。**这决定了下游能不能合法地把它当作 model effect 处理**——如果你把一个 crude rate 标成 `model_effect`，下游会把它误转成 Cox HR 之类的尺度。八选一：
  - `"model_effect"` — 论文用回归/Cox/logistic/LMM 等**显式统计模型**拟合的效应（HR、aOR、β、aHR、adjusted MD 等）。这是默认意义的"边"。
  - `"crude_rate"` — 粗事件率 / incidence rate / proportion / 频率（例：10/100 person-years）。**不是** model 估计。
