...
### 规则 5a: `has_numeric_estimate` 必须如实标注

下游 `filter_low_quality_edges` 会丢弃既无 effect_value、又无 CI、又无 theta_hat、又无 p_value 的边，**除非**该边显式标了 `has_numeric_estimate: false`。

- 论文只在正文里说 "no significant difference"、"减少趋势" 但**没有任何数值** → `has_numeric_estimate: false`，`estimate / ci / p_value` 全填 `null`，**不要**编一个 0 或 "ns"。
- 论文报告了 group means ± SD 但没给 mean difference / β / p → `has_numeric_estimate: true`（因为有 SD/SE 可下游计算），但 `estimate` 留 `null`，并在 `notes` 里写 "group means only, no between-group estimate".
- 凡是发现自己写不出来 estimate / ci / p_value 任意一个时，先回原文确认是不是真的没有；确认后，**如实**把 `has_numeric_estimate` 标 false，让下游用合理方式处理它，而不是装作有数。
  ...
  "edges": [
    {
      ...
      "source_context": "Table 2 row 3 或 Figure 3 panel B 或 Methods paragraph 2",
      "evidence_text": "从论文逐字摘抄的一句话/表格行，包含该数值，≤300字符",
      "statistic_type": "model_effect 或 crude_rate 或 group_mean 或 within_group_change 或 between_group_effect 或 sensitivity 或 subgroup 或 unknown",
...
- **statistic_type**（**新增，必填**）：标记这条 edge 的"统计量身份"。**这决定了下游能不能合法地把它当作 model effect 处理**——如果你把一个 crude rate 标成 `model_effect`，下游会把它误转成 Cox HR 之类的尺度。10 选一：
  - `"model_effect"` — 论文用回归/Cox/logistic/LMM 等**显式统计模型**拟合的效应（HR、aOR、β、aHR、adjusted MD 等）。这是默认意义的"边"。
  - `"crude_rate"` — 粗事件率 / incidence rate / 频率（例：10/100 person-years）。**不是** model 估计。
  - `"proportion"` — 比例 / 百分比（例："80% of patients achieved remission"、"5/8 patients responded"）。常见于**单臂 / 病例系列 / 描述性**研究。**不是** model 估计也**不是** rate（无 person-time 分母）。
  - `"descriptive_estimate"` — 描述性估计（中位数、四分位、众数等），不来自正式模型。常见于**病例系列 / 单臂干预**的疗效报告。
  - `"group_mean"` — 论文只报告各组均值（±SD），没给出组间差/回归系数。
  - `"within_group_change"` — 单组前后变化量（baseline → follow-up），不是组间比较。
  - `"between_group_effect"` — 组间差异（mean difference、risk difference 等），但**不是**回归 fit 出来的（如纯 ANOVA / t-test 算的差）。
  - `"sensitivity"` — 敏感性分析、不同模型规格的稳健性检查（不是主分析的代表）。
  - `"subgroup"` — 亚组分析（在主分析基础上分层）。
  - `"unknown"` — 你看了原文也判断不准（**必须**在 notes 里说明你看到的是什么）。
- **evidence_text**（**新增，必填**）：从论文中**逐字摘抄**包含该 edge 数值的最关键一句话或一行表格。≤300 字符。下游审计用它做"数字是否有出处"的可追溯检查。
  - ✅ `"Adjusted hazard ratio for sleep duration ≥10 h vs 7 h was 1.28 (95% CI 1.06–1.55)."`
  - ✅ `"Table 2, row 'TRE': mean change −3.2 (95% CI −4.4, −2.0) vs control −0.5"`
  - ❌ `"In Table 2"` — 太短，无法回溯
  - ❌ 自己改写过的句子 — 必须保持原文表述
- **source_context**（**新增，建议填**）：edge 在论文里的**结构化定位**。比 `source` 字段更细，便于自动审计。格式：`"Table 2 row 3"`、`"Figure 3 panel B"`、`"Section Results paragraph 4"`、`"Methods paragraph 2"` 等。
...
- **statistical_method**: 识别论文对该边使用的实际统计模型。仔细阅读方法部分。常见值：
  ...
  - `"descriptive"` — **新增**：纯描述性统计（mean / median / proportion / IQR），无任何回归 / 假设检验。**单臂病例系列、病例报告**应填这个。
  - `"none"` — **新增**：论文未做任何统计建模（极少见）。配合 `statistic_type=descriptive_estimate / proportion`。
  - `"other"` — 如果以上都不适用，在notes中说明

**⚠️ 关于单臂 / 病例系列的 statistical_method**：
当论文是 **case series / single-arm trial / case report**（无对照组）时：
- 如果只报告 X% 患者达到 Y 终点 → `statistical_method="descriptive"`，`statistic_type="proportion"`
- 如果只报告中位 / 平均某指标 → `statistical_method="descriptive"`，`statistic_type="descriptive_estimate"`
- **不要**填 `"linear"` / `"logistic"` 凑数——这些模型隐含了组间比较，会让下游推出错误的因果识别假设。

- **adjustment_variables**: 模型调整的协变量/混杂因素列表。从论文的方法或表注中提取。如果论文说"调整了年龄、性别、BMI、吸烟和教育"，写作 `["age", "sex", "BMI", "smoking", "education"]`。如果未调整或未说明，写作 `[]`。

### 规则 8: Edge 优先级标注

为每个 edge 标注 `priority` 字段，用于后续筛选。**`priority` 必须填写**，且**只能**取下列三个字符串之一（注意全部小写）：
...
**❌ 禁止**：`priority: "?"`、`priority: null`、`priority: "primary/secondary"`、缺失字段。下游 `_final_schema_enforcement` 会把任何非白名单值改写成 `"secondary"`，但你**仍然要**主动填写正确的值；让下游兜底意味着所有 exploratory edge 都会被当成 secondary 保留下来。
...
19. **`statistic_type` 是否如实分类**？特别注意：
    - Table 2 里 "incidence rate per 100 person-years" 是 `crude_rate`，**不是** `model_effect`，下游不会把它转成 Cox HR。
    - "intervention group: pre 6.4 → post 6.1" 是 `within_group_change`，不是 `between_group_effect`。
    - "Mean (SD) of TRT group: 130 (15)" 是 `group_mean`。
    - "5/8 patients (62.5%) achieved remission" 是 `proportion`，**不是** `model_effect`——单臂 / 病例系列里这种最多。
    - "Median symptom score 3.2 (IQR 2-4)" 是 `descriptive_estimate`，无对照所以不能转成 MD。
    - 来自 Cox/logistic/LMM 等回归 fit 的 HR/aOR/β 才是 `model_effect`。
22. **如果是单臂 / case series / case report 论文**（无对照组、无随机化）：
    - 不要把 X 写成 "intervention vs control"（论文里没 control）。
    - 不要让 statistic_type 是 `model_effect`——只能是 `proportion / descriptive_estimate / unknown`。
    - 不要让 statistical_method 是 `Cox / linear / logistic`——填 `descriptive` 或 `none`。
20. **`evidence_text` 是否逐字抄了原文 + ≤300 字符**？不要改写，不要省略数值。
21. 同一篇论文里多条 edge 的 `evidence_text` 是否各自指向具体语句（而不是都填 "Table 2"）？
